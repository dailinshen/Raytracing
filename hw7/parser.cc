...
//                string aw_file = "/Users/lindashen/Downloads/Ray-Tracer-master/Test-files/oamp.obj";
                ...
                std::vector<Triangle *> objectfile_triangles;
                ...
                for(unsigned int i=0; 3*i+2<tris_list.size();i++){
                    ...
                    thisobjectTriangle = new Triangle(t_ax,t_ay,t_az,t_bx,t_by,t_bz,t_cx,t_cy,t_cz);
//                    , 3*tris_list[3*i], 3*tris_list[3*i+1], 3*tris_list[3*i+2]);
//                    thisSurface = new Triangle(t_ax,t_ay,t_az,t_bx,t_by,t_bz,t_cx,t_cy,t_cz);
                    
                    thisobjectTriangle->set_type('t');
                    
                    Point a, b, c;
                    a.setPosition(t_ax,t_ay,t_az);
                    b.setPosition(t_bx,t_by,t_bz);
                    c.setPosition(t_cx,t_cy,t_cz);
                    
//                    Vector N = cross_product(b - a, c - a);
//                    N.normalize();
                    
//                    vertex_normal[tris_list[3*i]] = vertex_normal[tris_list[3*i]] + N;
//                    vertex_normal[tris_list[3*i + 1]] = vertex_normal[tris_list[3*i + 1]]+ N;
//                    vertex_normal[tris_list[3*i + 2]] = vertex_normal[tris_list[3*i + 2]] + N;
                    
                    thisobjectTriangle->setIndex((int)(surface_list.size() + objectfile_triangles.size()));
                    thisobjectTriangle->setMaterial(lastMaterial);
                    thisobjectTriangle->setSurfaceMin(boxminX, boxminY, boxminZ);
                    thisobjectTriangle->setSurfaceMax(boxmaxX, boxmaxY, boxmaxZ);
                    thisobjectTriangle->setbbox(*thisBbox);
...
//                    thisSurface->setbbox(*thisBbox);
//                    surface_list.push_back(thisSurface);
                    objectfile_triangles.push_back(thisobjectTriangle);
                    
                }
                Vector *vertex_normal = new Vector[v_list.size()];
//                surface_list.push_back(objectfile_triangles);
                for(int i=0; i<objectfile_triangles.size();++i){
                    vertex_normal[3 * tris_list[3 * i]] += objectfile_triangles[i]->getSurfaceNormal();
                    vertex_normal[3 * tris_list[3 * i + 1]] += objectfile_triangles[i]->getSurfaceNormal();
                    vertex_normal[3 * tris_list[3 * i + 2]] += objectfile_triangles[i]->getSurfaceNormal();
                }
                for (int i = 0; i < v_list.size(); ++i){
                    vertex_normal[i].normalize();
                }
                for(int i = 0; i < objectfile_triangles.size(); ++i) {
                    Vector N1 = vertex_normal[3 * tris_list[3 * i]];
                    Vector N2 = vertex_normal[3 * tris_list[3 * i + 1]];
                    Vector N3 = vertex_normal[3 * tris_list[3 * i + 2]];
//                    std::cout<<N2.getX()<<";"<<N2.getY()<<";"<<N2.getZ()<<std::endl;
                    
                    objectfile_triangles[i]->set_vertex_normal(N1, N2, N3);
                    
                    surface_list.push_back(objectfile_triangles[i]);
...
Imf::Rgba Readscene:: rec(Ray& r, float min_t, float max_t, int ray_type, int recurse_limit, int lightIndex, int shadowRayNum, Ray& sNormal){
    ...
    if(surfaceIntersect){
        ...
        Triangle *object_triangle = dynamic_cast<Triangle *>(surface_list[surfaceIndex]);
        if (surface_list[surfaceIndex]->get_type() == 't') {
            smoothNormal.setDirection(object_triangle->get_smooth_normal());
            surfaceNormal = smoothNormal;
//            std::cout<<object_triangle->getA().getX()<<std::endl;
