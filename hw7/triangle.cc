...
Triangle::Triangle(float xa, float ya, float za, float xb, float yb, float zb, float xc, float yc, float zc, int a_index_, int b_index_, int c_index_){
    a.setPosition(xa, ya, za);
    b.setPosition(xb, yb, zb);
    c.setPosition(xc, yc, zc);
    this->setSurfaceNormal(a, b, c);
...
float Triangle:: intersection(Ray& r){
    ...
    N = N1 * (1 - gamma - beta) + N2 * (beta) + N3 * (gamma);
    
    N = N * (1 / sqrt(dot_product(N, N)));
...
//    std::cout<<this->normal.getDir().x<<";"<<this->normal.getDir().y<<";"<<this->normal.getDir().z<<std::endl;
