...
bool Sphere::intersect (const Ray &r, double &t, Point &ipoint, Vector &normal, bool &isSphere) const
{
    ...
    if (discriminant > 0.0001) {
        ...
        if (temp_t > 0.0001 and (t <= 0.0001 or temp_t < t)) {
            t = temp_t;
        }
    }
    if (t <= 0.01) return false;    // intersections are behind the camera.
